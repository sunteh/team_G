...
public class BackgroundEnemyService implements Runnable{
	...
	public BackgroundEnemyService(Enemy enemy) {
		...
		try {
			if(Player.stage == 1) {
				image = ImageIO.read(new File("image/backgroundMapService4_1.png"));
				}
				else if(Player.stage == 2)
				{
					image = ImageIO.read(new File("image/backgroundMapService4_2.png"));
				}
				else if(Player.stage == 3)
				{
					image = ImageIO.read(new File("image/backgroundMapService4_3.png"));
				}
		} catch (Exception e) {
	...
	public void run() {
		while(enemy.getState()==0) {
			...
			if(rightColor.getRed() == 0 && rightColor.getGreen() == 0 && rightColor.getBlue() == 255 && bottomColor2.getBlue() == 255)
			{
				enemy.setRight(false);
				if(!enemy.isLeft())
				{
			...
			else if(leftColor.getRed() == 0 && leftColor.getGreen() == 0 && leftColor.getBlue() == 255 && bottomColor2.getBlue() == 255)
			{
				enemy.setLeft(false);
				if(!enemy.isRight())
				{
